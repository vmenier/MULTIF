...
def ParseDesignVariables_Plain (filename):
	...
	try:
	    ...
	    raise IOError("  ## ERROR : Could not open %s\n\n" % filename)
	...
	for line in fil:
		...
		if len(hdl) != 1 :
		    raise IOError("  ## ERROR : Unexpected input design variables format (line %d of %s).\n\n" % (id_line, filename));
...
def ParseDesignVariables_Dakota (filename):
	...
	if NbrVar < 0 :
	    raise IOError("  ## ERROR : No variables given in %s\n\n" % filename);	
	
	if NbrLin < id+NbrVar+1 :
	    raise IOError("  ## ERROR : Wrong variable definition in %s\n\n" % filename);
	...
	if NbrOut < 0 :
	    raise IOError("  ## ERROR : No output function given in %s\n\n" % filename);
	
	if NbrLin < id+NbrOut+1 :
	    raise IOError("  ## ERROR : Wrong output function  definition in %s\n\n" % filename);
...
def WriteOutputFunctions_Plain (filename,outputList):
		
    try:
        fil = open(filename, 'w')
    except:
        raise IOError("  ## ERROR : Could not open output file %s\n" % filename)
