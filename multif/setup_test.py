...
setup(name="toto", ext_modules=[Extension("_mshint_module",
      sources=["./mshint/boule.c"      , \
			...
			"./mshint/mesh.c" ,\
			"./mshint/scalem.c"] , \
			extra_compile_args=["-O3","-c","-Wuninitialized","-Wunused","-Winline","-Wshadow", \
			"-fexpensive-optimizations","-funroll-loops"])
		#	extra_compile_args=["-std=c99","-Wno-unused-variable","-Wno-unused-result"])
])
setup(name='hw',
       ext_modules=[Extension(name='hw', sources=['./mshint/toto.f'])],
       )
