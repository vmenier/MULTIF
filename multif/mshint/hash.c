...
int hashelt_3d(pMesh mesh) {
  ...
  for (k=1; k<=mesh->ne; k++) {
    ...
    for (i=0; i<4; i++) {
      ...
      i2 = idirt[i][1];
      i3 = idirt[i][2];
      mins = I_MIN(pt->v[i1],pt->v[i2]);
      mins = I_MIN(mins,pt->v[i3]);
      maxs = I_MAX(pt->v[i1],pt->v[i2]);
      maxs = I_MAX(maxs,pt->v[i3]);
  ...
  for (l=4*mesh->ne; l>0; l--) {
    ...
    while ( ll != inival ) {
      ...
      if ( sum1 == sum ) {
        mins1 = I_MIN(pt1->v[i1],pt1->v[i2]);
        mins1 = I_MIN(mins1,pt1->v[i3]);
        if ( mins1 == mins ) {
          maxs1 = I_MAX(pt1->v[i1],pt1->v[i2]);
          maxs1 = I_MAX(maxs1,pt1->v[i3]);
          if ( maxs1 == maxs ) {
            /* adjacent found */
