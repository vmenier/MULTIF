...
pBucket newBucket_3d(pMesh mesh,int nmax) {
  ...
  for (k=1; k<=mesh->np; k++) {
    ppt = &mesh->point[k];
    ii = I_MAX(0,(int)(dd * ppt->c[0])-1);
    jj = I_MAX(0,(int)(dd * ppt->c[1])-1);
    kk = I_MAX(0,(int)(dd * ppt->c[2])-1);
...
int buckin_3d(pMesh mesh,pBucket bucket,double *c) {
  ...
  ii = I_MAX(0,(int)(dd * c[0])-1);
  jj = I_MAX(0,(int)(dd * c[1])-1);
  kk = I_MAX(0,(int)(dd * c[2])-1);
  ic = (kk*siz + jj)*siz + ii;
  ...
  do {
    imin = I_MAX(0,ii-d);
    imax = I_MIN(ii+d,siz-1);
    jmin = I_MAX(0,jj-d);
    jmax = I_MIN(jj+d,siz-1);
    kmin = I_MAX(0,kk-d);
    kmax = I_MIN(kk+d,siz-1);
...
int buckin_2d(pMesh mesh,pBucket bucket,double *c) {
  ...
  if ( bucket->head[ic] ) {
    ...
    do {
      ...
      jmax = jj+d;
      imin = I_MAX(0,imin);
      imax = I_MIN(imax,siz-1);
      jmin = I_MAX(0,jmin);
      jmax = I_MIN(siz-1,jmax);
      for (j=jmin; j<=jmax; j++)
        for (i=imin; i<=imax; i++) {
